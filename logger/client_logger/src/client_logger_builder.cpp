...
logger_builder &
client_logger_builder::add_file_stream(const std::string &stream_file_path,
                                       logger::severity severity) & {
    if (stream_file_path.empty()) {
        std::cerr << "Warning: File path is empty, stream not added"
                  << std::endl;
        return *this;
    }
    ...
    try {
        ...
        std::cerr << "Error creating directory for log file: " << e.what()
                  << std::endl;
        return *this;
    }
    ...
    for (const auto &stream : severity_entry.first) {
        if (stream._stream.first == stream_file_path) {
            ...
            std::cerr << "Warning: File path '" << stream_file_path
                      << "' already added for this severity level" << std::endl;
            duplicate = true;
            break;
        }
    }
    {
        ...
        if (!test_file.is_open()) {
            std::cerr << "Warning: Cannot open log file for writing: "
                      << stream_file_path << std::endl;
            return *this;
        }
    }

    // comment if want to disallow duplicates
    severity_entry.first.emplace_front(
...
logger_builder &client_logger_builder::transform_with_configuration(
    ...
    if (node.contains("severity")) {
        ...
        for (auto &[severity_name, severity_config] :
             node["severity"].items()) {
            logger::severity severity;

            if (severity_name == "trace")
                severity = logger::severity::trace;
            else if (severity_name == "debug")
                severity = logger::severity::debug;
            else if (severity_name == "information")
            ...
            else if (severity_name == "critical")
            ...
            if (severity_config.contains("console") &&
                severity_config["console"].get<bool>()) {
                add_console_stream(severity);
            }

            if (severity_config.contains("files") &&
                severity_config["files"].is_array()) {
        ...
        for (const auto &stream : node["streams"]) {
            ...
            if (severity_str == "trace")
                ...
                severity = logger::severity::debug;
            else if (severity_str == "information")
                severity = logger::severity::information;
...
logger *client_logger_builder::build() const {
    if (_output_streams.empty()) {
        std::cerr << "Warning: No output streams configured, logs will not be "
                     "output anywhere"
                  << std::endl;
    }

    if (_format.empty()) {
        std::cerr << "Warning: Empty format string, using default format \"%m\""
                  << std::endl;
        return new client_logger(_output_streams, "%m");
    }
    if (_format.find("%m") == std::string::npos) {
        std::cerr << "Warning: Format string does not contain message "
                     "placeholder (%m), messages will not be displayed"
                  << std::endl;
    }
...
logger_builder &client_logger_builder::set_format(const std::string &format) & {
    if (format.empty()) {
        std::cerr
            << "Warning: Empty format string provided, using default \"%m\""
            << std::endl;
        _format = "%m";
    } else {
        _format = format;
...
logger_builder &
client_logger_builder::set_destination(const std::string &format) & {

    std::cerr << "Warning: set_destination is not applicable for client_logger"
              << std::endl;
    return *this;
}
